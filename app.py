...
@st.cache_data
def load_data():
    normal_df = pd.read_csv("clean_normal_expression.csv")
    cell_df = pd.read_csv("clean_cellline_expression.csv")
...
# Build dropdown options (Gene + Protein Description)
...
if not lookup_options:
    st.sidebar.warning("No targets match the selected Chromosome and Disease filters.")
    selected_label = None
else:
    selected_label = st.sidebar.selectbox(
        ...
        index=0,
        help="Type or search by Gene Symbol, Ensembl ID, or Protein Name."
    )
...
if selected_label:
    ...
    # 4. CLEAN HEADER & ELEGANT METADATA CHIPS
    st.markdown(f'<div class="main-header">{gene_info["Gene"]} — {gene_info.get("Gene description", "Protein Product")}</div>', unsafe_allow_html=True)

    dysreg_badge_class = "badge-alert" if "Elevated" in str(gene_info.get("Dysregulation_Status", "")) else "badge-blue"

    st.markdown(f"""
    <div class="tag-container">
        <span class="badge-gray"><b>Ensembl:</b> {gene_info.get('Ensembl', 'N/A')}</span>
        <span class="badge-gray"><b>Genomic Locus:</b> Chr {gene_info.get('Chromosome', 'N/A')}</span>
        <span class="badge-gray"><b>Tissue Specificity:</b> {gene_info.get('RNA tissue specificity', 'N/A')}</span>
        <span class="badge-gray"><b>Main Location:</b> {subcell_loc}</span>
        <span class="{dysreg_badge_class}"><b>Disease Involvement:</b> {disease_val}</span>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown(
    f'<div class="sub-header">Protein Product: <b>{gene_info.get("Gene description", gene_info["Gene"])}</b> | '
...
with tab1:
    ...
    with st.expander("View Comprehensive Biological & Protein Metadata ℹ️ ", expanded=True):
        ...
        with col_a:
            ...
            st.write(f"• **Ensembl ID:** {gene_info.get('Subcellular main location', 'N/A')}")
            st.write(f"• **Chromosomal Locus:** Chr {gene_info.get('Chromosome', 'N/A')}")
        with col_b:
            st.write(f"• **Protein Name:** {gene_info.get('Gene description', gene_info['Gene'])}")
